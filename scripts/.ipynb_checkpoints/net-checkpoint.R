...
journal2journal <- t(bi_mat) %*% bi_mat

journal_net <- as.network(journal2journal, directed = FALSE)

write.table(journal2journal,
  file = "/scratch/midway3/timothyelder/journal2journal_mat.txt")

rm(journal2journal)

write.table(bi_mat,
  file = "/scratch/midway3/timothyelder/author2journal_mat.txt")
...
desc_stats <- authors_df

desc_stats.to_csv("data/authors_desc_stats.csv")
...
ggsave("figures/author_scores_dens.pdf", all.g)
